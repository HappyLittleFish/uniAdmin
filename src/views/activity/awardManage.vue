  ...
  <div class="app-container">
    ...
    <el-table :key='tableKey' :data="list" v-loading="listLoading" border fit highlight-current-row
      ...
      <el-table-column class-name="status-col" :label="$t('table.status')" width="100">
        <template slot-scope="scope">
          <el-tag :type="scope.row.status | statusFilter">{{statusOptions[scope.row.status].label}}</el-tag>
...
export default {
  ...
  data() {
    return {
      ...
      statusOptionsNum: [0, 1, 2, 3],
      // statusOptions: ['未开始', '进行中', '已结束', '已禁用'],
      statusOptions: [{
        value: '0',
        label: '未开始'
      }, {
  ...
  filters: {
    statusFilter(status) {
      const statusMap = ['info', 'success', 'danger', 'primary']
  ...
  methods: {
    ...
    handleUpdate(row) {
      ...
      rowData.status = this.statusOptions[row.status]['label']
